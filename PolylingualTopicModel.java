...
public class PolylingualTopicModel implements Serializable {
	...
	public void addInstances (InstanceList[] training) {
		...
		for (int language = 0; language < numLanguages; language++) {
			...
			System.out.println(betas[language]);
	 ...
	 */
	public ArrayList<ArrayList<double[]>> getNormalizedTopicWordWeights(int topic) {
		System.out.println("Printing out getNormalizedTopicWordWeights(int topic)");
		ArrayList<ArrayList<double[]>> normalizedTokenWeightsList = new ArrayList<ArrayList<double[]>> ();
		for (int language = 0; language < numLanguages; language++) {
		    ArrayList<double[]> tokenWeights = new ArrayList<double[]>();
		    for (int type = 0; type < vocabularySizes[language]; type++) {
		        int[] topicCounts = languageTypeTopicCounts[language][type];//topicModel.typeTopicCounts[type];
		        double weight = betas[language];
		        int index = 0;
		    ...
		    normalizedTokenWeightsList.add(normalizedTokenWeights);
		}
		return normalizedTokenWeightsList;
	}
	...
	public static void main (String[] args) throws IOException {
		...
		ArrayList<ArrayList<double[]>>  normalizedTokenWeightsList = new ArrayList<ArrayList<double[]>> ();
		normalizedTokenWeightsList = topicModel.getNormalizedTopicWordWeights(0);
		System.out.println(normalizedTokenWeightsList);
	}
